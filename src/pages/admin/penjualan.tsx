        ...
        return (
          <>
            {row?.requirement &&
              (() => {
                try {
                  const parsedRequirement = JSON.parse(row.requirement);
                  return parsedRequirement.map((item: any, index: number) => (
                    <div key={index} className="flex gap-2 items-center">
                      <span
                        className="text-blue-400 max-w-40 cursor-pointer hover:underline line-clamp-1	 text-sm"
                        onClick={() => {
  ...
  useEffect(() => {
    if (searchParams.get('name')) {
      getData.setParams((prev: any) => ({
        ...
        filters: {
          ...prev.filters,
          namaPaket: searchParams.get('name'),
        },
      }));
    }
  }, []);
