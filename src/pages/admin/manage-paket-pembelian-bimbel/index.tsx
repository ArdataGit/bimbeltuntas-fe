...
export default function UserIndex() {
  ...
  const getList = useGetList({
    url: 'admin/paket-pembelian-bimbel/get',
    initialParams: {
      skip: 0,
      take: 10,
      sortBy: 'createdAt',
  ...
  const handleDeleted = async (id: number) => {
    FetchAPI(deleteData(`admin/paket-pembelian-bimbel/remove/${id}`)).then(
      () => {
        getList.refresh();
      }
    );
  ...
  const columns = [
    ...
    {
      title: 'Tanggal Dilaksanakan',
      colKey: 'date',
      align: AlignType.Center,
      sorter: true,
      cell: ({ row }: any) => {
        return <span>{moment(row.date).format('DD/MM/YYYY HH:mm')}</span>;
      },
    },
    ...
    {
      title: 'Mini Test',
      colKey: 'bankSoalCategoryId',
      align: AlignType.Center,
      cell: ({ row }: any) => {
        return (
          <p
            onClick={() => navigate(`/manage-latihan/${row.paketLatihan?.id}`)}
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <Tag
          ...
          icon={statusNameListMap[row.status as keyof StatusNameListMap]?.icon}
        >
          <span className="ml-1">
          ...
          <div className="flex justify-center gap-5">
            <Button
              ...
              onClick={() => {
                setDetail(() => ({
                  ...row,
                  paketLatihanId: row.paketLatihan?.id,
