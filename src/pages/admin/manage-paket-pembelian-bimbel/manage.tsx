...
export default function ManagePaketPembelian({
  ...
  const handleSubmit = async (data: any) => {
    ...
    const payload = {
      ...data,
      materiLink: data.materiLink || '',
      rekamanLink: data.rekamanLink || '',
      videoLink: data.videoLink || '',
      paketPembelianId: Number(id),
      paketLatihanId: Number(data.paketLatihanId),
      date: moment(data.date, 'DD/MM/YYYY HH:mm'),
    };

    console.log(payload);

    if (!payload.paketLatihanId) delete payload.paketLatihanId;
