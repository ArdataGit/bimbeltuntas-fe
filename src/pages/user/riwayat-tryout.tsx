...
export default function RiwayatTryout({ isBimbel }: any) {
	...
	const handleExportPDF = async (id: number, val: any) => {
		await getPdf(
			`file/generate-pdf/${id}`,
			`Tryout - ${moment(val.createdAt).format("DD-MM-YYYY HH:mm")}`,
			{
				id: id,
			},
		);
						...
						<>
							...
							<Popup
								...
								<Button
									...
									onClick={() => handleExportPDF(row.id, row)}>
									PDF
								</Button>
							</Popup>
							<Popup
