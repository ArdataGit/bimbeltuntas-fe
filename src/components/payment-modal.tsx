...
export default function PaymentModal({
  ...
  const uploadFile = async (e: any) => {
    ...
    const result = await postData('file/upload', formData);
  ...
  const handlePayment = async () => {
    ...
    const payload = {
      paketPembelianId: Number(itemDetail.id),
    ...
    if (
      itemDetail.requirement &&
      itemDetail.requirement !== '' &&
      itemDetail.requirement !== '[]' &&
      !payment.gambar
    ) {
      setIsLoading(false);
      return toast.error('Mohon Upload file persyaratan');
    }
    const images = [];

    if (payment.gambar) {
      for (const key of Object.keys(payment.gambar)) {
        const res = await uploadFile(payment.gambar[key]?.file[0]);
        images.push({
          file: res.data.url,
          text: payment.gambar[key]?.text,
        });
      }
    }

    if (images.length > 0) {
      payload.requirement = JSON.stringify(images);
    }

    postData('user/payment-gateway/create-payment', payload)
      .then((res) => {
  ...
  const requirement = JSON.parse(itemDetail.requirement);
  ...
  return (
    ...
    >
      ...
      itemDetail.requirement !== '[]' ? (
        <>
          <h5 className="text-md font-semibold mb-2">
            <span className="text-red-500">*</span> Upload Persyaratan
          </h5>
          <Form disabledEnter onSubmit={() => {}} className="space-y-6 mb-4">
            {requirement.map((item: any, index: number) => (
              <Input
                name={`gambar[${index}]`}
                type="file"
                multiple
                ...
                validation={{
                  required: ' Persyaratan Harus di isi',
                }}
              />
            ))}
          </Form>
