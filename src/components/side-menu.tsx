...
import LOGO2 from '@/assets/Logobaru.png';
...
export default function SideMenu({
  ...
  useEffect(() => {
    if (account && !account?.name && location.pathname !== '/profile') {
      toast.error('Silahkan lengkapi data diri anda terlebih dahulu');
      setTimeout(() => {
        ...
        <nav className="-mx-3 space-y-1 ">
          {menuList.map((menu: any) => {
            const menuItem = menu.pages.map((page: MenuItem) => (
              <button
                className={`w-full  text-left flex items-center group !mb-3 px-3 py-3 text-black justify-between transition-colors duration-300 transform rounded-lg  hover:bg-indigo-900 hover:text-white hover:shadow-xl ${
                  ...
                  >
                    ...
                    {menuOpened?.includes(item.paketPembelianId) && (
                      ...
                      >
                        ...
                        0 ? (
                          <Link
                            to={`/my-class/${item.paketPembelianId}/tryout?type=Tryout`}
                            className={`flex items-center mt-2 justify-between w-full px-3 py-2 text-base font-medium text-gray-600 transition-colors duration-300 transform rounded-lg hover:bg-gray-100 ${
                              checkRouteActive(
                        ...
                        <Link
                          to={`/my-class/${item.paketPembelianId}/tryout?type=Latihan`}
                          className={`flex items-center mt-2 justify-between w-full px-3 py-2 text-base font-medium text-gray-600 transition-colors duration-300 transform rounded-lg hover:bg-gray-100 ${
                            checkRouteActive(
                              `my-class/${item.paketPembelianId}/tryout?type=Latihan`,
