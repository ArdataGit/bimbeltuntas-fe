...
import { Checkbox, Dialog, Radio, Tag } from 'tdesign-react';
...
export default function OnGoingTryout({ isPembahasan, isBimbel }: any) {
  ...
  const getDetail = async () => {
    getData(`user/find-tryout/${tryoutId}`, { isPembahasan }).then((res) => {
      ...
      setIsRaguRagu(res?.soalId?.[onShow]?.isRaguRagu);
  ...
  const markAsRagu = async () => {
    postData(`user/tryout/mark-ragu`, {
      soalId: soalId?.[onShow]?.id,
      tryoutId,
      value: !isRaguRagu,
    }).then((res) => {
      ...
      setIsRaguRagu(!isRaguRagu);
      setSoalId((prev: any) => {
        prev[onShow].isRaguRagu = !isRaguRagu;
        return prev;
      });
    });
  };
  ...
  const getJawabanCorrect = (soal: any) => {
    let jawabanBenar = '-',
      jawabanKamu = '-';

    soal.jawabanShow?.forEach((item: any, index: number) => {
      if (item.isCorrect) {
        jawabanBenar = `${Number(index + 1)}`;
      }

      if (item.id === soal.jawabanSelect) {
        jawabanKamu = `${Number(index + 1)}`;
      }
    });
  ...
  const jawabanCorrect = getJawabanCorrect(soal);
  const [isRaguRagu, setIsRaguRagu] = useState(false);

  useEffect(() => {
    setIsRaguRagu(soalId?.[onShow]?.isRaguRagu);
  }, [onShow]);
        ...
        <div className="block md:flex flex-row-reverse">
          <div className="relative w-full md:w-3/12 md:border-l md:pl-10">
            <aside className="sticky top-10  border-[#DDD] ">
              <div className="flex  gap-y-5 justify-between mb-5 md:flex-row md:items-center header-section w-full">
                <div className="title">
                  {soalId[0]?.isRaguRagu ? 'RAGU' : 'GARAGU'}
              ...
              <ul className="grid grid-cols-5 md:grid-cols-2 xl:grid-cols-6 gap-1">
                {soalId?.map((item: any, index: number) => (
                  <li key={index}>
                    <button
                      onClick={() => setOnShow(index)}
                      className={`
                    text-indigo-950 hover:shadow-md text-center p-1 rounded-md w-full
                    ${
                      item.isRaguRagu && soalId?.[onShow]?.id != item.id
                        ? 'bg-[#ff9900] border-2'
          ...
          <div className="w-full md:w-9/12 md:pr-5 mt-10 md:mt-0">
            <div className="flex flex-col border-b pb-2 border-gray-300 gap-y-5 md:flex-row md:items-center justify-start md:justify-between header-section w-full">
              <div className="title flex gap-2 items-center">
                <img className="w-auto h-16 mr-2  " src={LOGO} alt="" />
                ...
                {soal.category && (
                  <Tag variant="light-outline">{soal?.category}</Tag>
                )}
              </div>
            ...
            <div className="min-h-[200px] mt-8 ">
              ...
              <Checkbox
                ...
                onChange={markAsRagu}
              >
                <p className="text-base">Tandai sebagai ragu-ragu ?</p>
              </Checkbox>
              {isPembahasan && (
                <p className="mb-4 mt-4 border-t pt-4 border-b pb-4 border-gray-300">
                  Jawaban Kamu:{' '}
                  <span
                    className={`${
                      jawabanCorrect.jawabanBenar === jawabanCorrect.jawabanKamu
                        ? 'text-green-500'
                        : 'text-red-500'
                    }`}
                  >
                    {jawabanCorrect.jawabanKamu !== '-'
                      ? String.fromCharCode(
                          64 + Number(jawabanCorrect.jawabanKamu)
                        )
                      : '-'}
                  </span>
                  <br />
                  ...
                  <span className="text-green-500">
                    {jawabanCorrect.jawabanBenar !== '-'
                      ? String.fromCharCode(
                          64 + Number(jawabanCorrect.jawabanBenar)
                        )
                      : '-'}
                  </span>
                </p>
